...
function insertarNovedad($novedad) {
    switch ($novedad) {
        case "Peso": {
                $sql = 'INSERT INTO t_movimientos (IdTipoMovimiento, IdAnimal, Fecha,FechaRegisto, Cantidad, Descripcion, idorg) VALUES (1,' . $_POST['IdAnimal'] . ',"' . $_POST['Fecha'] . '","' . $_POST['Fecha'] . '",' . $_POST['Cantidad'] . ',"' . $_POST['Descripcion'] . '",'.$_SESSION['idorg'].')';
                //print $sql;
                //exit();
                consultaSQL($sql, $_SESSION['idorg']);
                break;
