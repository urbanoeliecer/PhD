...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($tabla) {
        ...
        case "t_terceros": {
                $sql = 'INSERT INTO ' . $tabla . ' (IdTiposTercero, Tercero, Documento, Telefono) VALUES (' . $_POST['IdTiposTercero'] . ',"' . $_POST['Tercero'] . '","' . $_POST['Documento'] . '","' . $_POST['Telefono'] . '");';
                consultaSQL($sql);
                break;
            }
        case "t_tiposanimal": {
        ...
        case "t_animales": {
                if (isset($_SESSION['Genealogia'])) {
                    ...
                    $sql = 'INSERT INTO ' . $tabla . ' (IdLote, IdTipoAnimal, IdRaza, Numero, Nombre, FechaNacimiento, Estado) VALUES (' . $_POST['IdLote'] . ',' . $_POST['IdTipoAnimal'] . ',' . $_POST['IdRaza'] . ',"' . $_POST['Numero'] . '","' . $_POST['Nombre'] . '","' . $_POST['FechaNacimiento'] . '",1);';
                    consultaSQL($sql);
                    ...
                    $sql = 'INSERT INTO ' . $tabla . ' (IdLote, IdTipoAnimal, IdRaza, Numero, Nombre, FechaNacimiento) VALUES (' . $_POST['IdLote'] . ',' . $_POST['IdTipoAnimal'] . ',' . $_POST['IdRaza'] . ',"' . $_POST['Numero'] . '","' . $_POST['Nombre'] . '","' . $_POST['FechaNacimiento'] . '");';
        ...
        case "t_usuarios": {
                ...
                if ($IdPerfil == '') {
                    ...
                    $idOrgNew = $f[0];
                    $pass = 'bovAgro';
                    $sql = 'INSERT INTO ' . $tabla . ' (IdLicencia, IdPerfil, Usuario, Pass, Nombre1, Nombre2, Apellido1, Apellido2, Documento, email, IdOrg) VALUES (' . $_SESSION['IdLicencia'] . ',' .$IdPerfil. ',"' . $_POST['Usuario'] . '", sha2("' . $pass . '",256),"' . $_POST['Nombre1'] . '","' . $_POST['Nombre2'] . '","' . $_POST['Apellido1'] . '","' . $_POST['Apellido2'] . '","' . $_POST['Documento'] . '","'.$_POST['email'].'","'.$idOrgNew.'");';
                    consultaSQL($sql);
        ...
        case "t_lotes": {
                $sql = 'INSERT INTO ' . $tabla . ' (Idfinca, Lote) VALUES (' . $_POST['IdFinca'] . ',"' . $_POST['Lote'] . '");';
                consultaSQL($sql);
                break;
            }
    }
}
//print '$novedad '.$novedad.' Generalogia '.$_SESSION['Genealogia'].' $novedad '.$novedad;
...
if ($novedad == '' && $varSalto == 1)
    header('Location:../IniciarUso.php');
if ($novedad == '')
    //header('Location:../IniciarUso.php');
...
function fnctnrestaurarpass($mailrecibe,$nameenvia,$mailenvia,$asunto,$user,$newpass)
{
    //.fnctnobtfrmfch(1,
    	//'.fnctngenfooter($raiz).'
    ///print $vrbldatesrvdr = fnctnobtnfchsrvdr(0);
    $raiz = "http://www.agroit.co";
     
    print $cuerpo = '<html><head>   </head>
	<body style="font-family:Verdana,Arial,Tahoma;font-size:13px;color:#333;">
	<div style="width:100%;overflow:hidden;background-color:#4285F4;padding:5px 2%;font-size:15px;font-weight:bold;color:#FFF;">Restauración de su Contraseña</div>
	<div style="width:100%;padding:5px 1% 0 1%;">
	<p>El día '.$vrbldatesrvdr.' se realizó; una Solicitud de Restauración de su Clave de Acceso al Sistema FORZA.</p>
	<p style="color:#8c2426;font-weight:bold;">¡Si usted no realizó esta solicitud de cambio de contraseña, por favor envienos un correo informandonos sobre el caso!</p>
	<p>Para ingresar el acceso al sistema por favor utilice los datos dados a continuación en la dirección: <a href="'.$raiz.'/panel/index.php" rel="Regístrese">https://www.forza.com.co/panel</a></p>
    ...
    print phpversion();
    print $headers.="X-Mailer: PHP/" . phpversion() . "\r\n";
    # Envio el correo  
