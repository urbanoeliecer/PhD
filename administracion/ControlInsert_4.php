...
function insertarNovedad($novedad) {
    switch ($novedad) {
        case "Peso": {
                $sql = 'INSERT INTO t_movimientos (IdTipoMovimiento, IdAnimal, Fecha, Cantidad, Descripcion, idorg) VALUES (1,' . $_POST['IdAnimal'] . ',"' . $_POST['Fecha'] . '",' . $_POST['Cantidad'] . ',"' . $_POST['Descripcion'] . '",'.$_SESSION['idorg'].')';
                //print $sql;
                //exit();
                consultaSQL($sql, $_SESSION['idorg']);
                break;
        ...
        case "Leche": {
                $sql = 'INSERT INTO t_movimientos (IdTipoMovimiento, IdAnimal, Fecha, Cantidad, Descripcion,idorg) VALUES (2,' . $_POST['IdAnimal'] . ',"' . $_POST['Fecha'] . '",' . $_POST['Cantidad'] . ',"' . $_POST['Descripcion'] . '",'.$_SESSION['idorg'].')'; 
                consultaSQL($sql, $_SESSION['idorg']);
                break;
            }
...
function fnctnrestaurarpass($mailrecibe,$nameenvia,$mailenvia,$asunto,$user,$newpass)
{
	...
	<p>El día '.$fchIng.' se realizó una solicitud en AgroIT</p>
