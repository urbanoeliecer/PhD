...
function generarAgregar($pagina) {
    ...
    if (strcmp($pagina, "animales") == 0) {
        ...
        echo '<input type="submit" name="Traslado" value="Traslado">';  //170519       
...
function generarTablaEditar($tabla, $id, $idorg) {
    ...
    //150615
    echo '<div align="center">Id: '.$id.'</div>';
...
function generarTablaAgregar($tabla, $novedad = NULL, $idorg,$FechaNacimiento = NULL) { //170602 se agregó un nuevo parámetro
    $idanimal = @$_SESSION['UltimoAnm']; //170602
    ...
    //    $nmrcampos--;
    if (($tabla == 't_animales') && (@$_GET['FechaNacimiento'] != '')) $nmrcampos--; //170602
    for ($index = 0; $index < $nmrcampos; $index++) {
        $metadatos = mysqli_fetch_field_direct($p, $index);
    ...
    echo '<input type="hidden" name="FechaNacimiento" value="'.@$_GET['FechaNacimiento'].'">'; //170602
...
function generarFilaTablaAgregar($campo, $novedad = null, $hijo = NULL,$idorg) {
    ...
    switch ($tipo) {
        ...
        case MYSQLI_TYPE_NEWDECIMAL: { //0 246
                echo '<input type="number" name="' . $columna . '" step="0.01" min="0.00" width="100" ';
                if ($columna == 'Cantidad') echo ' value="1"'; 
                echo ' required>';
                break;
            }
        case MYSQLI_TYPE_VAR_STRING: { //253
                if (strcmp($columna, "Pass") == 0) {
                    ...
                    if (strcmp($columna, "email") == 0) {
                        ...
                        echo '<input type="text" name="' . $columna . '" maxlenght="' . $campo->length . '" size="18" value="-" required></input>';
                    }
                }
                break;
            }
        case MYSQLI_TYPE_LONG: { //3
                if (strpos($columna, "Id") !== FALSE) {
                    ...
                    //} else 
                    //170522 Debe mostrar solo los activos
                    {
                    ...
                    if ($novedad != 'Palpacion' && $columna == 'Numero'){
                        ...
                        $x = $f[0]+2;
                        echo ' value="'.substr($x,2,strlen($x)).'"';
                    }
                    else {
                        if ($columna == 'HembrasVivas') echo ' value="1"'; 
                        else echo ' value="0"';
                    }
                    echo $columna;
                    echo '>';
...
function getCombo($tabla, $nombre, $novedad = NULL, $default = Null, $defid = Null, $idorg) {
    ...
    if ($novedad == 'Palpacion'){ 
        $sql = "select IdAnimal, Numero from t_animales WHERE Estado = 1  and idtipoanimal = 5 and idorg = $idorg order by numero"; 
    }
